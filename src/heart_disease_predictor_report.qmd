...
jupyter:
  kernelspec:
    display_name: 'Python [conda env:heart_disease_predictor]'
    language: python
    name: conda-env-heart_disease_predictor-py
toc: true
...
As shown in Figure @fig-numeric-distributions, the numeric features of the dataset have varying distributions, which provide an overview of the data's spread and tendencies.

#| label: fig-numeric-distributions
#| fig-cap: "Distribution of numeric features in the dataset."
#| out-width: 100%
include_graphics("results/raw_feature_distributions.png")

The target variable (`num`) is distributed as displayed in Figure @fig-target-distribution, where the majority of the samples are classified into the lower categories of heart disease.

#| label: fig-target-distribution
#| fig-cap: "Distribution of the target variable (`num`)."
#| out-width: 100%
include_graphics("results/target_variable_distribution.png")

For categorical features, the grouped distributions based on the presence of heart disease are shown in Figure @fig-categorical-distributions. These stacked bar plots illustrate the relationship between categorical variables and the target variable.

#| label: fig-categorical-distributions
#| fig-cap: "Distribution of categorical features grouped by heart disease (`num`)."
#| out-width: 100%
include_graphics("results/categorical_features_distribution.png")

Boxplots for selected numeric features grouped by the target variable (num) are presented in Figure @fig-boxplots-class, offering insights into the spread and central tendencies of the selected features for each heart disease category.

#| label: fig-boxplots-class
#| fig-cap: "Boxplots of selected numeric features grouped by heart disease (`num`)."
#| out-width: 100%
include_graphics("results/raw_boxplots_by_class.png")
...
#| label: fig-logreg-coefficients
#| fig-cap: "Feature coefficients for Logistic Regression model."
#| out-width: 100%
include_graphics("results/log_reg_feature_coefficients.png")
...
Interestingly, females (`sex` = 0) are more likely to be free of heart disease, as indicated by the large negative coefficient for `onehotencoder__sex_0.0`. In contrast, males (`sex` = 1) are more likely to have heart disease, as reflected by the high positive coefficient for `onehotencoder__sex_1.0`. This is supported by Regitz-Zagrosek and Gebhard's research(2023)[@Regitz-Zagrosek2023], which highlights how biological sex differences, such as premenopausal women having a relative protection from coronary artery disease.
